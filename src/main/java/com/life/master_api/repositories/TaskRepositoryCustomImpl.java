...
public class TaskRepositoryCustomImpl implements TaskRepositoryCustom {
    ...
    public List<Task> findByCategory(Category category) {
        ...
        Join<Object, Object> categoryJoin = task.join("categories", JoinType.INNER);
    ...
    @Override
    public Page<Task> findByCategoryAndUser(Category category, User user, Pageable pageable) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        ...
        Predicate userPredicate = cb.equal(task.get("user"), user);
        ...
        if (pageable.getSort().isSorted()) {
            ...
            pageable.getSort().forEach(sort -> {
                if (sort.isAscending()) {
                    orders.add(cb.asc(task.get(sort.getProperty())));
                } else {
                    orders.add(cb.desc(task.get(sort.getProperty())));
                }
            });
        ...
        Root<Task> taskCount = countQuery.from(Task.class);
        Join<Object, Object> categoryJoinCount = taskCount.join("categories", JoinType.INNER);
        
        countQuery.select(cb.count(taskCount));
        Predicate categoryPredicateCount = cb.equal(categoryJoinCount, category);
        Predicate userPredicateCount = cb.equal(taskCount.get("user"), user);
